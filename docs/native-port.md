...
- **Lecteur natif — points à durcir** (Phase 2c faite, à affiner) :
  ...
  - **Mise en arrière-plan / veille — ✅ corrigé (chantier 5, 2026-05-31)** :
    - **(a) Veille intempestive** : résolu — `FLAG_KEEP_SCREEN_ON` géré
      dynamiquement via `updateKeepScreenOn()` : posé sur l'event `Playing`,
      retiré sur `Paused`/`Stopped`. Avant : posé une seule fois dans `load()`
      et perdu au retour d'arrière-plan. Après le fix, chaque cycle
      pause/reprise restaure le flag correctement via les events VLC.
    - **(b) Écran noir au retour d'arrière-plan/veille** : résolu — `handleOnPause`
      appelle maintenant `mediaPlayer.detachViews()` (+ `viewsAttached = false`)
      avant que la `Surface` soit détruite par Android. Le nouveau `handleOnResume`
      override appelle `mediaPlayer.attachViews(videoLayout, ...)` (reconnecte
      libVLC à la nouvelle Surface recréée par Android) puis `mediaPlayer.setTime(pos)`
      (force le décodage et l'affichage d'une frame à la position courante —
      sans ça, libVLC reste sur un plan noir en état `paused` après reattach).
      La reprise audio+vidéo reste manuelle (pas d'auto-resume).
...
| 2026-05-31 | Chantier 2 — Polish UX Accueil (apps natives + Electron, vitrine intouchée) : Hero (dot actif → barre de progression `hero-progress` 7 s synchro auto-advance, Ken Burns `hero-ken` 9 s, contenu fade-up `hero-rise` staggered 80 ms eyebrow/title/meta/desc/actions, garde `prefers-reduced-motion`) ; cartes hover adoucies (ring blanc 2 px → ring fin 1 px + halo `--accent-glow` + ombre portée, `cardFocused` TV conservé) ; rails fade-up `IntersectionObserver` seuil 0.15 + classe `.rail` interne + stagger `:nth-child` plafonné à 240 ms (carte 9+) + fallback IO absent ; header rail enrichi `.rowTitleGroup` (divider 14×1 px + compteur tabular-nums monospace) sur les 4 rails, masqué `≤640 px`. | ✅ Livré (build/lint OK, validation visuelle Electron utilisateur) |
| 2026-05-31 | Chantier 3 — Panneau « Épisodes » dans le lecteur (séries, **toutes plateformes**) : nouveau type `SeriesContext` dans `PlayerState` (seriesId/title?/currentSeason/currentEpisodeNum/tmdbId?) posé par `SeriesDetail.handlePlayEpisode` ET `Player.handlePlayEpisode`. `Player.tsx` fetch `getSeriesInfo` + stills TMDB par saison à la demande (cache `stillsBySeason`, anti-doublon `stillsInflightRef`). Chemin rapide via `seriesContext.tmdbId` (skip enrich) ; sinon fallback `enrichSeries(cleanTitle(name), extractYear(...) ?? releaseDate.slice(0,4))` → cache TMDB hit. 6 props relayées à `VideoPlayer` puis à `<TvPlayerOverlay>` (oubli initial bloquant → bouton invisible sur TV). Bouton « Épisodes » dans `.secondaryGroup` (icône stack `IconEpisodes`, label `S{n}·É{m}`) ; `panelKind` étendu à `'episodes'`. Overlay souris/tactile : panel inline `.epPanel` (sélecteur saison pills + grille horizontale `.epGrid` cartes 220×16:9 desktop / 168×16:9 mobile, vignettes Premium, badge « En cours », auto `scrollIntoView` sur `[data-current=true]`) ; navigation clavier interne `epFocus = { section, index }` synchronisée à focus DOM réel (`epSeasonChipsRef`/`epCardsRef`) → ArrowLeft/Right ±1, ArrowUp épisode→saison, ArrowDown saison→épisode courant, Enter active ; **court-circuit raccourcis player quand un panel quelconque est ouvert** (sinon flèches font seek±10 s). Overlay TV : `buildSections('episodes')` 2 sections (Saisons pills + Épisodes cartes 280 px 16:9 avec ring accent + badge « EN COURS »), ouverture focus directement sur l'épisode courant section 1, slide G/D dans section, H/B pour basculer. Bouton visible dès qu'`onPlayEpisode` est posé (sans attendre `episodesBySeason.length > 0`) → état « Chargement des épisodes… » dans le panel si fetch en cours (sinon le bouton pop tardivement et l'utilisateur ne le voit jamais). i18n `player.episodes` / `player.nowPlaying` / `player.episodesLoading` (fr/en/es). Garde-fou opérationnel ajouté à CLAUDE.md §IV-26. | ✅ Livré (build/lint OK, validation visuelle Electron ; TV bloquée hors scope code, cf. ligne suivante) |
| 2026-05-31 | Re-test TV Samsung 2018 (UE_NU76xx Tizen 4.0) — diagnostic clean : `tz security-profiles list` confirme cohabitation de profils Distributor « Samsung Individual » (`Iptvax-TV`, `IPTVINCH`, `ALLE`, `IptvaxEmu2`) ET « default Tizen » (`Iptvax`, `IptvaxEmu` → `tizen-distributor-signer-new.p12`). Profil `ALLE` actif (Author + Distributor Samsung lié DUID). Cert/profile correctement configurés → cause `118012` à 23 % n'est **pas** côté config. Confirme la conclusion 2026-05-23 : **mur Partner Samsung sur les TV Tizen 4.0** depuis ~2024-2025, sideload Individual rejeté quelle que soit la signature. Pas de voie code restante. Émulateur Tizen TV reste le seul chemin validé pour ce shell ; déploiement TV Samsung réelle reporté à : (a) inscription Partner Samsung, ou (b) TV plus récente acquise. | 🛑 Re-confirmé (Samsung policy) |
| 2026-05-31 | Chantier 4 — Polish UX lecteur mobile (téléphone Android Capacitor + tout client ≤640px ; vitrine intouchée). **Gestes tactiles** : double-tap g/d = seek ±10s (`handleSurfaceTouchEnd`, flash `.tapFlashZone`, **silencieux si overlay caché**), single-tap = toggle overlay différé 320ms (distingue le double-tap). **Refonte overlay mobile** : contrôles centrés `.mobileCenterControls` (←10s/play pastille blanche/10s→), sliders verticaux latéraux `.mobileSideBar` (luminosité gauche via overlay `.brightnessOverlay` filtre, volume droite ; barres pleines arrondies sans thumb, pointer capture), bouton retour **×** `.mobileBackBtn` (prop `onBack` de `Player.tsx`, visible seulement overlay affiché), bottom bar recentrée (audio/CC/épisodes). **Volume système Android** : plugin maison `VolumeControl` (`VolumeControlPlugin.java` STREAM_MUSIC via AudioManager + ContentObserver `volumeChange`, `src/native/volumeControl.ts`) → le slider pilote le son média du tél + sync boutons physiques. **Sous-titres natifs stylés** : options libVLC déplacées au **constructeur LibVLC** (`buildEngine` — sub-text-scale/freetype-color/freetype-background-opacity/freetype-bold/sub-margin ; les media options `:option` sont ignorées), restyle à chaud via `rebuildEngine`+`:start-time`+réapplication pistes (`setSubtitleStyle`), mapping `NATIVE_SUB_*` dans `VideoPlayer`. Fix halo blanc (ne pas forcer `--freetype-outline-color=0` → contour blanc parasite ; ombre conditionnée au fond). **Lifecycle** : `handleOnPause` pause libVLC en arrière-plan/écran éteint + `FLAG_KEEP_SCREEN_ON` pendant lecture (wake lock JS `navigator.wakeLock` pour web/Electron). Fix flash bleu tap (`-webkit-tap-highlight-color`), events souris synthétiques WebView neutralisés (`touchHappenedRef`). Sous-titres rendus en inline styles = preview Personnaliser exacte. CLAUDE.md §XI mis à jour. | ⚠️ Livré (build/lint OK) — **2 régressions lifecycle restantes** : (a) veille parfois déclenchée malgré lecture ; (b) **écran noir au retour d'arrière-plan/veille alors que le son tourne** (surface libVLC perdue). Corrigées en chantier 5 (ligne suivante). |
| 2026-05-31 | Chantier 5 — Fiabiliser le lifecycle du lecteur natif Android (libVLC). **(a) Veille intempestive corrigée** : `updateKeepScreenOn()` nouvelle méthode thread-safe (capturée par `runOnUiThread`) — posée sur l'event libVLC `Playing`, retirée sur `Paused`/`Stopped`. Avant : flag posé une seule fois dans `load()`, perdu au retour d'arrière-plan. **(b) Écran noir au retour corrigé** : `handleOnPause` appelle désormais `mediaPlayer.detachViews()` + `viewsAttached = false` (libVLC se déconnecte proprement de la `Surface` avant qu'Android la détruise). Nouveau override `handleOnResume` : `mediaPlayer.attachViews(videoLayout, null, true, false)` (reconnecte libVLC à la nouvelle `Surface` recréée par Android) + `mediaPlayer.setTime(pos)` (force le pipeline de rendu à décoder et afficher la frame courante en état `paused` — sans ce seek, plan noir garanti). `handleOnDestroy` inchangé (sa garde `if (viewsAttached)` évite un double-detach). Reprise audio+vidéo reste manuelle (pas d'auto-resume). | ✅ Code livré — à valider sur device (cap sync + run android) |
| 2026-05-31 | Chantier 6 (incrément 1/2) — Polish UX mobile apps natives (téléphone Android Capacitor ; vitrine intouchée). **Safe-area edge-to-edge** : `targetSdk 35` (Android 15) force la WebView sous la barre de statut → header logo/profil à moitié masqué par l'heure/batterie. Fix : `viewport-fit=cover` dans `index.html` (active `env(safe-area-inset-*)`) + `var(--safe-top)` intégré dans `--topnav-h` (≤900/≤640) et dans le `top:` de tous les éléments fixes du haut (`.brand-fixed`/`.topnav`/`.profile-fixed`/`.search-fixed-mobile`). Web inchangé (`--safe-top`=0). **Brand mobile solidifiée** : capsule glass ronde 38px (cohérente avec recherche/profil) au lieu du logo nu flottant. **Hero portrait** : capé à `max-height: 52vh` (au lieu de 640px) → le 1er rail dépasse toujours, plus de hero qui mange tout l'écran. **Catalogue tactile** : grille posters Films/Séries `repeat(3→2, 1fr)` sur ≤640/≤380 (aligne le garde-fou §IV-20, posters larges lisibles au lieu d'une grille desktop rétrécie ; texte/étoile favori PreviewCard remontés en taille) ; chips `CategoryBar` à hauteur tactile 38px + `overscroll-behavior-x: contain` sur le rail (le scroll horizontal des catégories ne déborde plus sur la page). `tsc -b && vite build` OK. **Note vérif** : sur web pur l'app est la vitrine (`isVitrine`) → les pages catalogue/Home ne sont rendues que dans `AppGate` (natif/Electron), donc non observables en preview navigateur ; validation sur device. CLAUDE.md §IV-20 mis à jour. | ✅ Incrément 1 livré + validé device utilisateur |
...
| 2026-06-15 | **Téléchargements hors-ligne (Premium — Android & Windows)**. Télécharge le fichier direct Xtream COMPLET (toutes pistes) ; le lecteur natif (mpv/ExoPlayer) le lit hors-ligne. Flag `isDownloadCapable = isCapacitor \|\| isElectron`. Abstraction moteur `src/services/downloads/engine.ts` (electron/capacitor/noop) ; registre device-local (jamais Supabase). `DownloadsContext` (reflet moteur + filtre profil + gating Premium), `<DownloadButton>` (MovieDetail + épisodes SeriesDetail), page `MyDownloads` (`/telechargements`), bouton `ProfilePanel`. Lecture locale `file://` via `localPlayerState` (reprise = `historyId`), `useElectronPlayer` sans en-têtes HTTP pour source locale. Échappatoire hors-ligne dans `AppContent` (bouton « Accéder à mes téléchargements » si Xtream injoignable). Electron : `electron/downloads.cjs` (streaming Node + reprise Range + cache affiche + registre `userData/downloads/registry.json` + IPC). Capacitor : plugin `DownloaderPlugin.java` (Android DownloadManager + SharedPreferences). `tsc -b && vite build` OK, lint OK. | 🟡 Code livré — Electron à valider sur machine ; Android à valider sur device (`cap sync` + APK ; pause/reprise DownloadManager relance depuis le début en v1) |
| 2026-06-15 | **Fix téléchargements — étranglement Electron + blocage Android**. Bug desktop : connexion unique de longue durée → étranglée par le serveur Xtream (débit s'effondre puis abort ; pause/reprise manuelle redonnait du débit). Bug mobile : `DownloadManager` restait bloqué en `PENDING` indéfiniment (« En attente » sans jamais démarrer). **Fix commun = téléchargement SEGMENTÉ par tranches `Range` (8 Mo)** — chaque tranche = connexion courte dans la fenêtre de burst → débit constant ; + watchdog d'inactivité (~15 s → re-tranche) + auto-retry borné + redirections http↔https suivies à la main + repli flux-unique si `Range` ignoré (200). Electron : réécriture de `run()` dans `electron/downloads.cjs`. Android : **`DownloaderPlugin.java` réécrit** — `DownloadManager` remplacé par un téléchargeur en-process (`ExecutorService` + `HttpURLConnection` + `RandomAccessFile`, même boucle de tranches), vraie pause/reprise par `Range` (`.part` conservé), réconciliation `paused` au redémarrage. Tradeoff : plus de service système d'arrière-plan + notification. `node --check` OK (Electron), `:app:compileDebugJavaWithJavac` BUILD SUCCESSFUL, `tsc -b && vite build` OK. CLAUDE.md §XI mis à jour. | 🟡 Code livré — Electron à valider sur machine ; Android à valider sur device (`cap sync` + APK) |
| 2026-06-16 | **Téléchargements Electron — débit (connexions parallèles)**. Symptôme suite au fix précédent : plus d'abort, débit stable mais LENT (vs apps IPTV « rapides »). Cause : tranches `Range` téléchargées **séquentiellement** = une seule connexion à la fois → on subit le throttle par-connexion. Fix : `electron/downloads.cjs` réécrit en **téléchargeur parallèle** — `probeSource` (taille + support Range), puis `downloadParallel` = pool de `MAX_CONNECTIONS`=6 workers (env `IPTVAX_DL_CONNECTIONS`) tirant les tranches d'une file, `fetchChunk` écrit chaque tranche à sa **position** (`createWriteStream {flags:'r+', start}`). Le débit total est multiplié (chaque connexion a son propre token-bucket). Reprise via manifeste annexe `.prog.json` (tranches finies ; ignoré si `.part` disparu). Watchdog par-tranche (15 s) + watchdog GLOBAL (60 s → error). `rec.finished` : un seul transfert/id + `cancel` attend la fermeture des handles avant `unlink` (verrou Windows). Repli `downloadSingle` si serveur sans Range (200). Android inchangé (parallélisme pas encore porté). `node --check` OK, `tsc -b && vite build` OK, lint 0 erreur. CLAUDE.md §XI mis à jour. | 🟡 Code livré — à valider sur machine (tuner `IPTVAX_DL_CONNECTIONS` si fournisseur limite les connexions) |
| 2026-06-16 | **Fix barre qui oscille 0↔X% (téléchargement parallèle Electron)**. Symptôme : ~10 Mo en 1 s puis reset à 0, en boucle. Cause : sous charge multi-connexions, le serveur ferme chaque connexion **tôt** (livre < les 8 Mo demandés) ; l'ancien `fetchChunk` exigeait la tranche EXACTE, sinon rejetait et **décomptait** les octets (`downloaded -= got`) puis rejouait → boucle infinie sans progrès net. Fix : `fetchChunk`→`fetchRange` rend le nombre d'octets **réellement écrits** (positionnels, valides) ; le worker remplit chaque tranche depuis sa **position** via autant de connexions que nécessaire ; `downloaded` est **monotone** (jamais décompté) ; file de tâches `{i, pos}` (la position est conservée à la remise en file → pas de double comptage). Comportement aligné sur l'ancien moteur séquentiel (qui repartait toujours de `received`). `node --check` OK, module charge OK. CLAUDE.md §XI mis à jour. | 🟡 Code livré — à valider sur machine |
