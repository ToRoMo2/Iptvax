...
- **Lecteur natif — points à durcir** (Phase 2c faite, à affiner) :
  ...
  - **Mise en arrière-plan / veille — 2 RÉGRESSIONS OUVERTES (chantier 4, à
    traiter session suivante)** :
    - Partiellement traité : `handleOnPause` met libVLC en pause quand l'app
      passe en arrière-plan / l'écran s'éteint, et `FLAG_KEEP_SCREEN_ON` est
      posé pendant la lecture (+ wake lock JS `navigator.wakeLock` web/Electron).
    - **(a) Veille intempestive** : l'écran se met PARFOIS en veille malgré une
      lecture en cours → `FLAG_KEEP_SCREEN_ON` pas toujours effectif. Pistes :
      le flag est posé sur la fenêtre dans `load()` mais peut être perdu après
      certains cycles (recreate WebView, changement de fenêtre) ; envisager de
      (re)poser le flag aussi sur `Playing` et de le maintenir tant que
      `mediaPlayer.isPlaying()`. Vérifier que `getActivity().getWindow()` est
      bien la fenêtre visible.
    - **(b) Écran noir au retour d'arrière-plan/veille** : à la reprise, le SON
      fonctionne mais la vidéo reste **noire** → la `Surface` de la
      `VLCVideoLayout` est détruite quand l'app part en arrière-plan
      (`surfaceDestroyed`) et libVLC n'est pas ré-attaché à la nouvelle surface
      au retour. Piste : sur `handleOnResume`, ré-attacher les vues
      (`mediaPlayer.detachViews()` puis `attachViews(videoLayout, …)`), ou gérer
      le `SurfaceHolder.Callback` pour re-bind la surface ; éventuellement
      forcer un léger re-render (seek +0 / setVideoTrack) pour réamorcer le
      pipeline vidéo. C'est le point central de la prochaine session.
