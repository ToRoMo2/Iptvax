...
- **webOS** → `useWebOSPlayer` (HTML5 `<video>` + hls.js avec URL Xtream directe, sans ffmpeg ; pistes audio MP4/HLS exposées via `video.audioTracks` / `AUDIO_TRACKS_UPDATED` ; sous-titres v1 omis — Media Pipeline `luna://` requise pour les pistes embarquées MKV).
- **Défaut (web + Electron Option B)** → `usePlayer` (ffmpeg via `/api/*`, comportement historique).

La surface vidéo (`<div>` transparent vs `<video>` réel) conditionne sur `isCapacitor` — seul Android a besoin du `<div>` (libVLC derrière) ; webOS et web rendent un `<video>` standard. **Toujours passer par ces bascules** (`isCapacitor` pour la surface, dispatch hook dans `VideoPlayer.tsx` pour le moteur) — ne jamais appeler directement un hook de lecteur ailleurs.
