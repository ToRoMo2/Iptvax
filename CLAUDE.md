...
La surface vidéo (`<div>` transparent vs `<video>` réel) conditionne sur `player.usesNativeSurface` (générique) — `true` pour Capacitor toujours, `true` pour webOS uniquement quand la Pipeline luna:// est active (jamais le cas sur le simu actuel). **Toujours passer par ces bascules** (`usesNativeSurface` pour la surface, dispatch hook dans `VideoPlayer.tsx` pour le moteur) — ne jamais appeler directement un hook de lecteur ailleurs.
