...
25. **Sous-titres prod — ffmpeg 7.x obligatoire** : l'extraction `/api/subtitle` utilise `ffmpeg -c:s webvtt` pour convertir un sous-titre texte (SRT/ASS/MOV_TEXT) en WebVTT. ⚠ **ffmpeg 5.1.x (Debian Bookworm `apt install ffmpeg`)** a un bug d'extraction WebVTT depuis MKV : il sort en code 0 mais avec un VTT contenant uniquement l'en-tête, **sans cues** — les sous-titres ne s'affichent jamais côté UI. Le Dockerfile installe le build statique **BtbN ffmpeg 7.x** dans `/usr/local/bin/ffmpeg` (et ffprobe), que `proxy.cjs` détecte en premier via `pickBinary()`. Les binaires `ffmpeg-static`/`ffprobe-static` npm ne sont gardés que pour le dev local (Windows/macOS) — sur glibc Bookworm ils segfault sur tout input HTTP. Garde-fou côté serveur : `/api/subtitle` détecte un VTT sans `-->` (pas de cues) et **refuse de mettre en cache** — sinon un résultat dégradé reste collé 1 h. Si vous changez d'image de base Docker, vérifiez que le binaire dans `/usr/local/bin/ffmpeg` est bien ffmpeg ≥ 6.0 (`ffmpeg -version`).
